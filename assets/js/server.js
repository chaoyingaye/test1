...
var shopData=[
    ...
    {id:1001,alt:"饼干",icon:"./assets/img/1541389743203.jpg"},
    ...
    req.on("end",function () {
        ...
        switch (reqData.type){
            case 0:
                ...
                break;
            case 1:
                resData.result=removeItemShop(reqData.item);
                break;
            case 2:
                resData.result=changeItemNum(reqData.item,reqData.num);
                break;
            case 3:
                resData.result=checkItem(reqData.item,reqData.all,reqData.select);
                break;
            case 4:
...
server.listen(3009,"10.9.42.213",function () {
    console.log("服务已开启");
});
...
function checkItem(item,all,select) {
    if(all){
        shoppingList.map(function (t) {
            t.select=select;
        })
